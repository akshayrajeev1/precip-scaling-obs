...
high_res = 10
low_res = 100
distH = xr.open_mfdataset('/level02_indata/distzonal_'+str(high_res)+'km_10v.nc')
distL = xr.open_mfdataset('/level02_indata/distzonal_'+str(low_res)+'km_10v.nc')
pamtmapH=distH.pamt[:,1:]
pamtmapL=distL.pamt[:,1:]
bincrates=distH.precipitation_bin[1:]
...
xr.Dataset({"shift": bom,"error": errm}).to_netcdf(path='/data/bo_zonal_'+str(high_res)+'_'+str(low_res)+'km_10v.nc')